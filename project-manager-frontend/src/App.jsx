...
// Load Tailwind via CDN (standard practice for single-file web apps here)
const TailwindCSS = () => (
  <script src="https://cdn.tailwindcss.com"></script>
);

// --- CONFIGURATION ---
// Set the API base URL. Use the secure port (7001) for the C# backend.
...
      // Set simple user info from localStorage if token exists
      if (!user) {
...
    setToken(token);
    setUser({ username });
    // Header set by the useEffect/Axios interceptor
  };
...
    // NOTE: Changed to custom confirmation since window.confirm() is disallowed.
...
  // State for Scheduler Bonus Task
...
      setScheduleError("Cannot schedule an empty project. Add tasks first.");
      setLoading(false);
      return;
    }

    // Map existing tasks into the SchedulerTaskInput DTO format
    const tasksToSchedule = project.tasks.map(t => ({
      // IMPORTANT: We use placeholder values for complex fields not tracked in our basic TaskForm
      title: t.title,
      estimatedHours: 1, // Placeholder for the Scheduler API
      dueDate: t.dueDate, // Placeholder for the Scheduler API
      dependencies: [] // Placeholder: Tasks currently have no dependency field on the client-side
    }));

    try {
      const payload = { tasks: tasksToSchedule };
      // API_BASE_URL/v1/projects/{projectId}/schedule
      const response = await axios.post(`${API_BASE_URL}/v1/projects/${project.id}/schedule`, payload);
      
      setScheduleOrder(response.data.recommendedOrder);
      setScheduleError('');
    } catch (err) {
      console.error('Scheduler API Error:', err);
      // Handles custom C# error messages (e.g., Cycle detected)
      const apiError = err.response?.data?.Error || 'Failed to generate schedule due to a processing error.';
      setScheduleError(apiError);
    } finally {
      setLoading(false);
    }
  };
...
                disabled={loading || project.tasks.length === 0}
                className="px-4 py-2 bg-purple-600 text-white text-sm font-medium rounded-md shadow-md hover:bg-purple-700 transition disabled:opacity-50"
              >
...
          {scheduleOrder && scheduleOrder.length > 0 && (
            <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg shadow-inner">
              <h4 className="font-bold text-green-700 mb-2">✅ Recommended Task Order:</h4>
...
          {scheduleError && (
            <div className="mb-6 p-4 bg-red-100 border border-red-400 text-red-700 rounded-lg shadow-inner">
              <h4 className="font-bold mb-2">⚠️ Scheduling Error:</h4>
...
            <ul className="divide-y divide-gray-200">
                {/* START OF CORRECTION BLOCK */}
              {project.tasks.length === 0 && (
                <li key="no-tasks" className="p-6 text-gray-500 text-center">No tasks added yet.</li>
              )}
